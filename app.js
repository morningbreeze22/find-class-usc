...
app.post("/", async function (req, res) {
    ...
    for(let year=start;year<=end;year++){
        for(let semester=1;semester<=3;semester++){
            ...
            var course_id = pname+"-" + num;
            
            var time = year + " " + semes[semester];

            var resp = await getCourseInfo(pname,num,year,semester);
            resp=resp.match(new RegExp('<h2 class="single">' + "(.*)" + "</h2>"))[1];
            if(resp.includes("This course isn't available for")){
                respjson.push({
                    "id": course_id,
                    "time": time,
                    "held": "NO",
                ...
                resp=resp.match(new RegExp("(.*)" + '<span class="units">'))[1];
                respjson.push({
                    "id": course_id,
                    "time": time,
                    "held": "YES",
                    "name": resp,
                    "link": getUrl(pname,num,year,semester)
                })
            }
            var resptable = ConvertJsonToTable(respjson,"result","tab","");
        }

        var resptable = resptable +  '<br> <form action="/"><input type="submit" value="back" /></form>';
    }
    res.send(resptable);
});
